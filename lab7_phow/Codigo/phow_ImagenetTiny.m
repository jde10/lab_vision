...
conf.calDir = '/Users/JDLVF/Documents/MATLAB/imagenet_tiny' ;
conf.dataDir = '/Users/JDLVF/Documents/MATLAB/imagenet_tiny_data_200cat_30train_2_4_spatial/' ;
conf.autoDownloadData = false ;
conf.numTrain = 30 ;
conf.numTest = 30 ;
conf.numClasses = 200 ;
conf.numWords = 600 ;
conf.numSpatialX = [2 4] ;%NumSpatial!!!!
...
if ~exist(conf.calDir, 'dir') || ...
   (~exist(fullfile(conf.calDir, 'agaric'),'dir') && ...
    ~exist(fullfile(conf.calDir, 'train', 'agaric')))
...
if ~exist(fullfile(conf.calDir, 'agaric'),'dir')
...
timeElapsed_trainVocabulary = toc;
%% --------------------------------------------------------------------
...
timeElapsed_Histograms = toc;
%% --------------------------------------------------------------------
%                                                  Compute feature map
% --------------------------------------------------------------------
tic
psix = vl_homkermap(hists, 1, 'kchi2', 'gamma', .5) ;
timeElapsed_FeatureMap = toc;
%% --------------------------------------------------------------------
%                                                            Train SVM
% --------------------------------------------------------------------
tic
...
save([conf.resultPath 'TIMES.mat'],'timeElapsed_setupData', 'timeElapsed_trainVocabulary', 'timeElapsed_Histograms', 'timeElapsed_FeatureMap', 'timeElapsed_trainSVM', 'timeElapsed_testSVM') ;
